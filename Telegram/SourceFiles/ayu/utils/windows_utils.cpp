...
void processLegacy(const QString &appdata, const QString &iconPath) {
	auto shortcut = appdata + "/Microsoft/Internet Explorer/Quick Launch/User Pinned/TaskBar/SandyGram Desktop.lnk";
	if (!QFile::exists(shortcut)) {
		shortcut = appdata + "/Microsoft/Internet Explorer/Quick Launch/User Pinned/TaskBar/SandyGram.lnk";
	}
	if (!QFile::exists(shortcut)) {
		return;
	}
...
void processNewShortcuts(const QString &iconPath) {
	...
	const auto shortcut = path + u"SandyGram Desktop/SandyGram.lnk"_q;
	const auto native = QDir::toNativeSeparators(path).toStdWString();
...
void reloadAppIconFromTaskBar() {
	...
	QString iconPath = appdata + "/SandyGram.ico";
