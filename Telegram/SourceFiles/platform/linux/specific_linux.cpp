...
bool GenerateDesktopFile(
	...
	const auto sourceFile = u":/misc/com.sandygram.desktop.desktop"_q;
	...
	if (!Core::UpdaterDisabled()) {
		...
		if (!Core::Launcher::Instance().customWorkingDir()) {
			QFile::remove(u"%1sandygram.desktop._%2.desktop"_q.arg(
				targetPath,
				md5Hash));
		...
		QFile::remove(u"%1sandygram.desktop.%2.desktop"_q.arg(
			targetPath,
			md5Hash));
	}
...
void start() {
	QGuiApplication::setDesktopFileName([&] {
		...
		if (!Core::UpdaterDisabled()) {
			return u"com.sandygram.desktop._%1"_q.arg(
				Core::Launcher::Instance().instanceHash().constData());
		}

		return u"com.sandygram.desktop"_q;
	}());
