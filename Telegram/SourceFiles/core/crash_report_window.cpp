...
PreLaunchWindow::PreLaunchWindow(QString title) {
	...
	setWindowTitle(title.isEmpty() ? u"SandyGram"_q : title);
...
, _log(this)
, _close(this) {
	_label.setText(u"Could not start SandyGram Desktop!\nYou can see complete log below:"_q);
...
, _launch(std::move(launch)) {
	...
	if (_sendingState == SendingNoReport) {
		_label.setText(u"Last time SandyGram Desktop was not closed properly."_q);
	} else {
		_label.setText(u"Last time SandyGram Desktop crashed :("_q);
	}
	...
	_getApp.setText(u"GET THE LATEST VERSION OF SANDYGRAM DESKTOP"_q);
...
void LastCrashedWindow::saveReport() {
	QString to = QFileDialog::getSaveFileName(0, u"SandyGram Crash Report"_q, QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation) + u"/report.telegramcrash"_q, u"Telegram crash report (*.telegramcrash)"_q);
	if (!to.isEmpty()) {
		QFile file(to);
...
void LastCrashedWindow::updateControls() {
	...
	QSize s(2 * padding + QFontMetrics(_label.font()).horizontalAdvance(u"Last time SandyGram Desktop was not closed properly."_q) + padding + _networkSettings.width(), h);
	if (s == size()) {
		resizeEvent(0);
	} else {
		resize(s);
